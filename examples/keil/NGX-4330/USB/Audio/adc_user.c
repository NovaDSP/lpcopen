/***********************************************************************
* $Id:: adc_user.c 11131 2013-07-19 22:45:17Z nxp31782                 $
*
    ...
    if (pCtrl->SetupPacket.wIndex.W == 0x0200) {
        ...
        if ((pCtrl->SetupPacket.wValue.WB.L == 0) &&
            ...
            case AUDIO_MUTE_CONTROL:
                ...
                break;
            case AUDIO_VOLUME_CONTROL:
                pAdcCtrl->set_volume(pAdcCtrl, *((__packed uint16_t *)pCtrl->EP0Buf));
...
/*----------------------------------------------------------------------------
  ...
  pDTD->buffer2  - Original buffer pointer  
  pDTD->buffer3  - Received packet length  
  pDTD->buffer4  - Original transfer length  
 *----------------------------------------------------------------------------*/
void ADC_start_xfr(USB_ADC_CTRL_T* pAdcCtrl, ADC_SUBSTREAM_T* pSubs, uint32_t mode)
{
    ...
    for (i = 0; i < NUM_DTDS; i++) {
        ...
        /* Since the audio packets are smaller than 4KB, we will re-use
        buffer2, buffer3 and buffer4 members of TD struct to track original
        buffer location, rx packet length and original transfer size for ease
        of implementing the SW.
        */
        pDTD[i].buffer2 = ptrBuff;
        pDTD[i].buffer3 = 0;
        pDTD[i].buffer4 = pSubs->maxp;
        ptrBuff += pSubs->maxp;
    }
...
ErrorCode_t ADC_iso_in_hdlr(USBD_HANDLE_T hUsb, void* data, uint32_t event)
{
    ...
    if ((pAdcCtrl->flags & ADC_RECORDING) && (event == USB_EVT_IN)) {
        while ( (pDTD->total_bytes & TD_ACTIVE) == 0) {
            pDTD = &pSubs->head_TD[pSubs->rd_idx];
            /* store samples sent */
            pSubs->rd_count += ((pDTD->buffer4 - ((pDTD->total_bytes >> 16) & 0x7FFF)) >> 2);
            /* update read index */
            pSubs->rd_idx++;
            ...
            */ 
            if (pSubs->total_diff != 0) {
                if (pSubs->total_diff > 0){
                    total_bytes -= (SAMPLE_SIZE << 16);
                    pDTD->buffer4 = pSubs->maxp - SAMPLE_SIZE;
                    pSubs->total_diff--;
                } else {
                    total_bytes += (SAMPLE_SIZE << 16);
                    pDTD->buffer4 = pSubs->maxp + SAMPLE_SIZE;
                ...
                pDTD->buffer4 = pSubs->maxp;
            }
            /* anounce pkt is transmitted to the connector */
            if (LPC_OK != pAdcCtrl->usb_tx_done(pAdcCtrl, pSubs, pDTD)) {
                /* extit the interrupt handler */
                return LPC_OK;
            }
...
ErrorCode_t ADC_iso_out_hdlr (USBD_HANDLE_T hUsb, void* data, uint32_t event)
{
    ...
    if ((pAdcCtrl->flags & ADC_PLAYING) && (event == USB_EVT_OUT)) {
        while ( (pDTD->total_bytes & TD_ACTIVE) == 0) {
            ...
            pDTD->buffer3 = pDTD->buffer4 - ((pDTD->total_bytes >> 16) & 0x7FFF);
            pSubs->wr_count += (pDTD->buffer3 >> 2);
            /* incremnet write index */
            pSubs->wr_idx++;
            pSubs->wr_idx &= (NUM_DTDS - 1);
            /* anounce pkt arrival to the connector */
            pAdcCtrl->usb_rx_done(pAdcCtrl, pSubs, pDTD);
            /* clear and re-queue the buffer */
            pDTD->buffer0 = pDTD->buffer2;
            pDTD->buffer1 &= ~0xFFF;
            pDTD->next_dTD = TD_NEXT_TERMINATE;
...
ErrorCode_t ADC_SyncSOF_Event (USBD_HANDLE_T hUsb)
{
    ...
    if(++pAdcCtrl->sof_count >= pAdcCtrl->sof_interval) {
...
{
    USB_ADC_CTRL_T* pAdcCtrl = GET_ADC_CTRL(hUsb);
    /* reset the fractional PLL settings */
    pAdcCtrl->cur_frac = PLL_48K_FRACM;
    ...
    pAdcCtrl->subs[SUBS_SPEAKER].i2s_pos = 0;
    pAdcCtrl->subs[SUBS_MIC].wr_idx = pAdcCtrl->subs[SUBS_MIC].rd_idx = 0;
    pAdcCtrl->subs[SUBS_MIC].wr_valid_idx = DTD_INVALID_IDX;
    pAdcCtrl->subs[SUBS_MIC].i2s_pos = 0;
            
    /* disconnect the connector */
    pAdcCtrl->usb_rx_done = 0;
    pAdcCtrl->usb_tx_done = 0;
    pAdcCtrl->record = 0;
    pAdcCtrl->play = 0;
    pAdcCtrl->set_volume = 0;
    /* Free codec resource */
    Codec_disconnect(pAdcCtrl);
...
ErrorCode_t ADC_ASyncSOF_Event (USBD_HANDLE_T hUsb)
{
    ...
    if (pAdcCtrl->sof_count >= (1 << USB_ADC_SYNC_REFRESH)) {
        ...
        if (pAdcCtrl->flags & ADC_RECORDING) {
            ...
            /* calculate the sample we should tx with this referesh period */
            range = DEF_SYNC_VALUE >> (14 - USB_ADC_SYNC_REFRESH);
            /* compute fractional/extra samples to send/reduce to be sent to host */
            diff = (range - pSubs->wr_count);
    		/* track rolling  average of samples over/under */
            pSubs->total_diff += diff; 
        }
    ...
    USB_ADC_CTRL_T** ppAdcCtrl, uint32_t sync_mode)
{
    ...
    *ppAdcCtrl = pAdcCtrl;
    ...
    if ( sync_mode & ADC_ASYNC_MODE) {
        ...
        pAdcCtrl->subs[SUBS_SPEAKER].sync_data = DEF_SYNC_VALUE;
    }
    ...
    pAdcCtrl->subs[SUBS_MIC].wr_valid_idx = DTD_INVALID_IDX;
    /* no synch endpoint for capture */
    if ( sync_mode & ADC_ASYNC_MODE) {
        pAdcCtrl->subs[SUBS_MIC].sync_ep_QH = 0; 
        pAdcCtrl->subs[SUBS_MIC].sync_ep_bit = 0;        
        pAdcCtrl->subs[SUBS_MIC].sync_ep_num = 0;  
    }

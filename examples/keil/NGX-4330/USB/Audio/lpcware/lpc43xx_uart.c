	...
	* 0 < mulFracDiv <= 15, 0 <= dividerAddFracDiv <= 15 */
    ...
    if (best_error < UART_ACCEPTED_BAUDRATE_ERROR)
    {
        if (((LPC_UART1_Type*) UARTx) == LPC_UART1)
        {
            ...
            ((LPC_UART1_Type*) UARTx)->FDR = (UART_FDR_MULVAL(bestm) | UART_FDR_DIVADDVAL(bestd)) & UART_FDR_BITMASK;
        }
        else
        {
...
void UART_Init (LPC_USARTn_Type* UARTx, UART_CFG_Type* UART_ConfigStruct)
{
    ...
    {
        /* FIFOs are empty */
        ((LPC_UART1_Type*) UARTx)-> /*IIFCR.*/ FCR = (UART_FCR_FIFO_EN | UART_FCR_RX_RS | UART_FCR_TX_RS);

        // Dummy reading
        while ((((LPC_UART1_Type*) UARTx)->LSR & UART_LSR_RDR) || ((((LPC_UART1_Type*) UARTx)->IIR & UART_IIR_INTID_MASK) == UART_IIR_INTID_CTI))
        {
    ...
    else
    {
        ...
        // Dummy reading
        while ((UARTx->LSR & UART_LSR_RDR)  || ((UARTx->IIR & UART_IIR_INTID_MASK) == UART_IIR_INTID_CTI))
        {
    ...
    if (((LPC_UART1_Type*) UARTx) == LPC_UART1)
    {
        tmp = (((LPC_UART1_Type*) UARTx)->LCR & (UART_LCR_DLAB_EN | UART_LCR_BREAK_EN)) & UART_LCR_BITMASK;
    }
    else
    {
 ...
 * via defined symbol UART_BLOCKING_TIMEOUT.
 **********************************************************************/
uint32_t UART_Receive (LPC_USARTn_Type* UARTx, uint8_t* rxbuf, uint32_t buflen, TRANSFER_BLOCK_Type flag)
{
    uint32_t    bToRecv;
 ...
 * @return 		none
 **********************************************************************/
void UART_ABCmd (LPC_USARTn_Type* UARTx, UART_AB_CFG_Type* ABConfigStruct, FunctionalState NewState)
{
    uint32_t    tmp;
 ...
 * @return none
 **********************************************************************/
void UART_FullModemForcePinState (LPC_UART1_Type* UARTx, UART_MODEM_PIN_Type Pin, UART1_SignalState NewState)
{
    uint8_t tmp = 0;
 ...
 * @return none
 **********************************************************************/
void UART_FullModemConfigMode (LPC_UART1_Type* UARTx, UART_MODEM_MODE_Type Mode, FunctionalState NewState)
{
    uint8_t tmp = 0;
 ...
 * @return		None
 **********************************************************************/
uint32_t UART_RS485Send (LPC_USARTn_Type* UARTx, uint8_t* pDatFrm, uint32_t size, uint8_t ParityStick)
{
    uint8_t     tmp;
